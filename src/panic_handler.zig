...
//
// Notably, the simulator can only redraw the GUI console - and any newly printed errors -
// in it's main update loop!
//
...
//
// An idea is to save the return address when entering game code so if we panic
// we can jump back to the simulator as if we returned normally. This probably
// requires rewinding the stack somehow. Maybe taking a snapshot of all the
...
//
// I tried just using the return address, but I needed to rewind the stack as well to avoid
// smashing it. This is super fragile since it depends on how the code is generated by the compiler.
...
pub fn panic(
    ...
    while (true) {
        // Crashes the simulator. Disabled on windows
        // since I can't get logging to print to stdout.
        if (builtin.os.tag != .windows) @breakpoint();
    }
}
